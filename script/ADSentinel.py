...
from django.shortcuts import render  # Django 렌더링을 위해 추가
...
def run_nmap_scan(ip_address):
    ...
    nmap_result = subprocess.run(
        ["nmap", "-p", "22,53,88,135,137,138,139,389,445,464,636,3268,3269", ip_address],
        capture_output=True,
        text=True
    ).stdout
...
def run_inspection_check(vulc_id, ip_address):
    ...
    script_path = f"hd/script/vulc/{vulc_id}.sh"
...
def run_ADSentinel_scan(ip_address):
    ...
    matching_cves = []
    cve_risk_results = []
    scan_result = db["scan_results"].find_one({"ip_address": ip_address})
    if scan_result is not None:
        samba_version = scan_result.get("enum4linux_results", {}).get("samba_version", "")
    else:
        samba_version = ""
